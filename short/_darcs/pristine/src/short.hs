...
import Parser( shortspec, mkState )
import Syntax
import SyntaxPretty( prettyPrintSH )
import Flatten(insideOut, SH_FL_Spec(..), prettyPrintFlatSH)
import Merge (merge)
import TLACodeGen(groupSendInstr, gen)
import RewriteWhen(rewriteWhen)
import Rewrite (rewriteTag, rewriteSpecialOperators)
...
main = 
    do{ snames <- getArgs
      ...
      ; let e = filter (\en -> case en of 
		                 (Left err) -> True
		                 _ -> False) l
      ; if length e > 0
	then do{ let (Left err) = head e
	       ; putStrLn ("ERROR: " ++ show err)
               ; exitFailure
	       }
	else do{putStrLn "---- PARTS ----"
	       ; mapM_ ( \(Right (SH_Spec name cl)) -> 
			     putStrLn $ prettyPrintSH $ SH_Spec name cl) l
...
readAndParse fname = do{ fcontent <- readFile fname
