...
assoc :: AS_Expression -> Assoc
assoc (AS_InfixOP _loc op _a _b) = let (_, a) = infixPrec op table in a
...
ppVA (VA_RecType m) =
    let l = Map.foldWithKey (\(VA_String k) v acc -> 
              (text k <//> text ":" <+> 
              ppVA v):acc) [] m
     in brackets $ align (vcat (punctuate comma l))
ppVA (VA_SeqType v) =
